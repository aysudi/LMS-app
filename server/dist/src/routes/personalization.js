...
router.post("/wishlist/add/:courseId", authenticateToken, async (req, res) => {
    try {
...
router.delete("/wishlist/remove/:courseId", authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { courseId } = req.params;
        if (!userId) {
...
router.post("/search-history", authenticateToken, async (req, res) => {
    try {
        ...
        const existingIndex = user.searchHistory.findIndex((search) => search.query === query && search.category === category);
        if (existingIndex !== -1) {
...
// Add viewed course
router.post("/viewed/:courseId", authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { courseId } = req.params;
        if (!userId) {
        ...
        const existingIndex = user.viewedCourses.findIndex((viewed) => viewed.course?.toString() === courseId);
        if (existingIndex !== -1) {
...
router.get("/recommendations", authenticateToken, async (req, res) => {
    try {
        ...
        const viewedCategories = user.viewedCourses
            .map((viewed) => viewed.course?.category)
            .filter(Boolean);
