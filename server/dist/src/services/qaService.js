...
// Question Services
export const createQuestion = async (courseId, userId, questionData) => {
...
export const getQuestionsByCourse = async (courseId, query = {}) => {
    const { page = 1, limit = 10, lessonId, search, tags, answered = "all", sortBy = "newest", } = query;
...
export const getQuestionById = async (questionId, userId) => {
    ...
    if (userId) {
        ...
        question.userVoteType = question.getUserVoteType(userId);
        // Add userVoteType and isAccepted to answers
        answers.forEach((answer) => {
            answer.userVoteType = answer.getUserVoteType(userId);
            const isAccepted = question.acceptedAnswer?.toString() === answer._id.toString();
...
// Answer Services
export const createAnswer = async (questionId, userId, answerData) => {
    const question = await Question.findById(questionId);
    if (!question) {
        throw new Error("Question not found");
    }
...
export const updateAnswer = async (answerId, userId, updateData) => {
