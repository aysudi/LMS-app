...
import { createUserProfile, comparePassword, hashPassword, } from "../utils/user.utils";
import { generateVerificationToken, generatePasswordResetToken, hashToken, } from "../utils/token.utils";
...
const getRemainingLockTime = (lockUntil) => {
...
export const login = async (loginData) => {
    ...
    if (user.isBanned) {
        if (user.banExpiresAt && user.banExpiresAt > new Date()) {
            const remainingTime = Math.ceil((user.banExpiresAt.getTime() - Date.now()) / (1000 * 60 * 60));
            throw new Error(`Your account is temporarily banned. Ban expires in ${remainingTime} hour(s). Reason: ${user.banReason || "No reason provided"}`);
        }
            ...
            // Permanent ban
            throw new Error(`Your account is permanently banned. Reason: ${user.banReason || "No reason provided"}`);
        }
    }
    await checkAndUnlockAccount(user);
    if (user.lockUntil && user.lockUntil > new Date()) {
        const remainingTime = getRemainingLockTime(user.lockUntil);
        throw new Error(`Account is temporarily locked due to too many failed login attempts. Try again in ${remainingTime} minutes.`);
    }
    ...
    if (!isValidPassword) {
        ...
        const updateData = { loginAttempts: currentAttempts };
        if (currentAttempts >= MAX_ATTEMPTS) {
            updateData.lockUntil = new Date(Date.now() + LOCK_TIME);
            updateData.loginAttempts = 0;
            await User.findByIdAndUpdate(user._id, updateData);
            const lockTimeMinutes = Math.ceil(LOCK_TIME / (1000 * 60));
            throw new Error(`Too many failed login attempts. Account locked for ${lockTimeMinutes} minutes.`);
        }
        else {
            await User.findByIdAndUpdate(user._id, updateData);
            const remainingAttempts = MAX_ATTEMPTS - currentAttempts;
            throw new Error(`Invalid email or password. ${remainingAttempts} attempt${remainingAttempts === 1 ? "" : "s"} remaining before account lockout.`);
        }
    }
    const updateData = {
...
export const forgotPassword = async (email) => {
    ...
    if (!user.isEmailVerified) {
        throw new Error("Please verify your email address first before resetting password.");
    }
    if (user.authProvider !== "local") {
        throw new Error(`This account was created with ${user.authProvider}. Please sign in with ${user.authProvider}.`);
    }
...
export const banUser = async (userId, bannedBy, banDuration, // in hours
reason) => {
...
export const checkAndUnbanExpiredUsers = async () => {
    const now = new Date();
