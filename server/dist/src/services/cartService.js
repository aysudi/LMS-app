...
export const getUserCart = async (userId) => {
    let cart = await Cart.findOne({ userId }).populate({
        path: "items.courseId",
        select: "title description image category level originalPrice discountPrice isFree instructor rating ratingsCount enrollmentCount createdAt",
        populate: {
            path: "instructor",
    ...
    return {
        ...
        totalValue: cart.items.reduce((total, item) => total + item.priceAtTimeOfAdding, 0),
    };
};
export const addCourseToCart = async (userId, courseId) => {
    ...
    const existingItem = cart.items.find((item) => item.courseId.equals(courseObjectId));
    if (existingItem) {
    ...
    try {
        const updatedUser = await User.findByIdAndUpdate(userId, { $addToSet: { cart: courseObjectId } }, // $addToSet prevents duplicates
        { new: true });
        if (!updatedUser) {
...
export const removeCourseFromCart = async (userId, courseId) => {
    ...
    const itemIndex = cart.items.findIndex((item) => item.courseId.equals(courseObjectId));
    if (itemIndex === -1) {
    ...
    await User.findByIdAndUpdate(userId, { $pull: { cart: courseObjectId } }, { new: true });
    return {
        courseId,
        totalItems: cart.items.length,
        totalValue: cart.items.reduce((total, item) => total + item.priceAtTimeOfAdding, 0),
...
export const getCartSummary = async (userId) => {
    ...
    return {
        ...
        totalValue: cart?.items.reduce((total, item) => total + item.priceAtTimeOfAdding, 0) || 0,
    };
};
...
export const isInCartQuick = async (userId, courseId) => {
    try {
        const user = await User.findById(userId).select("cart");
        if (!user)
            return false;
