...
export const getAdminAnalyticsService = async (params = {}) => {
    ...
    const revenueData = (await Order.find()).reduce((acc, order) => {
    ...
    const previousPeriodStart = new Date(dateFilter.createdAt.$gte.getTime() -
    ...
    const previousRevenue = previousRevenueData[0]?.totalGross * platformCommissionRate || 0;
    ...
    const enrollments = {
        ...
        lastMonth: 0, // Calculate if needed
        growth: 0, // Calculate if needed
        completionRate: enrollmentStats[0].total[0]?.total > 0
