...
export const createOrder = async (req, res) => {
    try {
        const { items, billingAddress, couponCode, notes } = req.body;
        ...
        const alreadyEnrolled = courses.filter((course) => user?.enrolledCourses.some((enrolled) => enrolled._id.toString() === course._id.toString()));
    ...
    try {
        ...
        const [orders, total] = await Promise.all([
            Order.find({ user: userId })
                .populate("items.course", "title image instructor category originalPrice discountPrice")
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit),
...
export const getOrderById = async (req, res) => {
    try {
        ...
        const order = await Order.findOne({ _id: orderId, user: userId }).populate("items.course", "title image instructor category originalPrice discountPrice duration level");
        if (!order) {
...
export const completeOrder = async (orderId, paymentDetails) => {
    try {
        ...
        const enrollments = order.items.map((item) => new Enrollment({
            user: order.user,
            course: item.course,
            order: order._id,
