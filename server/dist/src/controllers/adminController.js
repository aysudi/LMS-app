...
export const updateUserStatus = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(targetUserId, { status }, { new: true }).select("-password -refreshToken -verificationToken -passwordResetToken");
        if (!user) {
...
export const updateUserRole = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(targetUserId, { role }, { new: true }).select("-password -refreshToken -verificationToken -passwordResetToken");
        if (!user) {
            return res.status(404).json({
                success: false,
                message: "User not found",
...
export const bulkUpdateUsers = async (req, res) => {
    try {
        ...
        const result = await User.updateMany({ _id: { $in: userIds } }, { $set: validUpdates });
        res.status(200).json({
            success: true,
