...
export const getUserEnrollments = async (req, res) => {
    try {
        ...
        const { status, page = "1", limit = "10", sortBy = "lastAccessedAt", sortOrder = "desc", } = req.query;
        if (!userId) {
        ...
        const [enrollments, total] = await Promise.all([
            Enrollment.find(filter)
                .populate({
                path: "course",
                select: "title description image instructor category originalPrice discountPrice totalLessons totalDuration rating studentsEnrolled",
                populate: {
                    path: "instructor",
...
export const updateEnrollmentProgress = async (req, res) => {
    try {
        const userId = req.user?.id;
        const { enrollmentId } = req.params;
        const { lessonId, completed, watchTime = 0, } = req.body;
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Authentication required",
...
export const getEnrollmentNotes = async (req, res) => {
    try {
        const userId = req.user?.id;
        const { enrollmentId } = req.params;
...
export const addEnrollmentNote = async (req, res) => {
    try {
        const userId = req.user?.id;
        const { enrollmentId } = req.params;
        const { lesson, content, timestamp } = req.body;
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Authentication required",
...
export const toggleLessonBookmark = async (req, res) => {
    try {
        const userId = req.user?.id;
        const { enrollmentId } = req.params;
...
export const getEnrollmentReviews = async (req, res) => {
    try {
        const userId = req.user?.id;
        const { enrollmentId } = req.params;
        if (!userId) {
...
export const getLearningStats = async (req, res) => {
    try {
        ...
        const enrollments = await Enrollment.find({ user: userId }).populate("course", "category");
        const totalEnrolledCourses = enrollments.length;
        const totalCompletedCourses = enrollments.filter((e) => e.status === EnrollmentStatus.COMPLETED).length;
        const totalInProgressCourses = enrollments.filter((e) => e.status === EnrollmentStatus.ACTIVE && e.progressPercentage > 0).length;
...
export const recalculateEnrollmentProgress = async (req, res) => {
    try {
        const userId = req.user?.id;
        const { enrollmentId } = req.params;
        if (!userId) {
...
export const recalculateAllUserProgress = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
...
export const enrollInFreeCourse = async (req, res) => {
    try {
        ...
        try {
            const courseWithInstructor = await Course.findById(courseId).populate("instructor", "_id firstName lastName");
            if (courseWithInstructor && courseWithInstructor.instructor) {
                ...
                const conversationResult = await findOrCreateConversation({
                    studentId: userId,
                    instructorId: courseWithInstructor.instructor._id,
                    courseId: courseId,
                });
                if (conversationResult.success) {
                    console.log("✅ Conversation created successfully:", conversationResult.data?._id);
                }
                else {
