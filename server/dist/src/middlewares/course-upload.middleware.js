...
export const uploadVideoToCloudinary = async (buffer, filename) => {
    return new Promise((resolve, reject) => {
        cloudinary.uploader
            .upload_stream({
...
export const courseUploadErrorHandler = (err, req, res, next) => {
    if (err instanceof multer.MulterError) {
        ...
        if (err.code === "LIMIT_UNEXPECTED_FILE") {
            return res.status(400).json({
                success: false,
                message: "Unexpected field. Only 'image' and 'videoPromo' fields are allowed.",
            });
        }
        return res.status(400).json({
            success: false,
...
export const processCourseUploads = async (req, res, next) => {
    try {
