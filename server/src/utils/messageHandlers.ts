...
export const registerMessageHandlers = (
  ...
  socket.on("conversation:join", (data: JoinConversationData) => {
    try {
  ...
  socket.on("message:stopTyping", (data: TypingData) => {
    try {
      if (!data.conversationId) {
        return;
      }
  ...
  socket.on("message:markAsRead", async (data: MarkAsReadData) => {
    try {
